...
### 保留5为小数
def round_list(lst):
    return np.round(lst, decimals=5).tolist()
...
def var_all_case(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_name="LA"):
    if dataset_name == "LA":
        ...
        # image_list = ["/home/leesoon/mnt/data/Medical/2018LA_Seg/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
        # image_list = ["./data/LA/2018LA_Seg/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
...
def var_test_all_case(model_name, model, num_outputs, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_name="LA", save_result=True,  test_save_path=None, metric_detail=1,  nms=0):
    if dataset_name == "LA":
        with open('./data/LA/test.list', 'r') as f:
            image_list = f.readlines()
        # image_list = ["/home/leesoon/mnt/data/Medical/2018LA_Seg/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
    ...
    for image_path in loader:
        ...
        dice.append(single_metric[0])
        jc.append(single_metric[1])
        hd.append(single_metric[2])
        asd.append(single_metric[3])
    ...
    print('dice std is {}'.format(np.std(dice)))
    print('jc std is {}'.format(np.std(jc)))
    print('hd std is {}'.format(np.std(hd)))
    print('asd std is {}'.format(np.std(asd)))
    
    with open(test_save_path+'../performance.csv', 'a+', encoding='utf-8') as csvfile:
        w = csv.writer(csvfile)
        w.writerow(round_list(dice))
        w.writerow(round_list(jc))
        w.writerow(round_list(hd))
        w.writerow(round_list(asd))

    with open(test_save_path+'../performance.txt', 'a') as f:
        f.writelines('average metric is {} \n'.format(avg_metric))
        f.writelines('dice std is {} \n'.format(np.std(dice)))
        f.writelines('jc std is {} \n'.format(np.std(jc)))
        f.writelines('hd std is {} \n'.format(np.std(hd)))
        f.writelines('asd std is {} \n'.format(np.std(asd)))
